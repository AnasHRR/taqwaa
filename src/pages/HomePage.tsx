...
export function HomePage() {
  ...
  return (
    <div className="page-content">
      {/* ===== HEADER ===== */}
      <header className="relative pt-6 pb-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-xl mx-auto px-4">
          <div className="flex flex-col items-center mb-4">
            <CrescentMoon className="w-14 h-14 crescent-shadow animate-float mb-2" />
            <h1 className="text-4xl font-bold font-[Amiri] text-gradient-gold tracking-wide leading-tight">
              تقوى
            </h1>
            <div className="flex items-center gap-3 mt-1">
              <div className="w-10 h-[1px] bg-gradient-to-r from-transparent to-gold-500/40" />
              <p className="text-midnight-200 text-xs tracking-widest uppercase" style={{ fontFamily: "'Cairo', sans-serif", letterSpacing: '0.15em' }}>
                Taqwaa
              </p>
              <div className="w-10 h-[1px] bg-gradient-to-l from-transparent to-gold-500/40" />
            </div>
            <p className="text-midnight-300 text-[11px] mt-1 font-light">
          ...
          <div className="flex justify-center mb-3">
            <div className="glass-card rounded-2xl px-7 py-4 flex flex-col items-center">
              <div className="flex items-baseline gap-1">
                <span className="text-4xl sm:text-5xl font-bold text-white font-mono tabular-nums tracking-wider">
                  {hours}
                </span>
                <span className="text-4xl sm:text-5xl font-bold text-gold-400 animate-pulse mx-0.5">:</span>
                <span className="text-4xl sm:text-5xl font-bold text-white font-mono tabular-nums tracking-wider">
                  {minutes}
                </span>
                <span className="text-4xl sm:text-5xl font-bold text-gold-400 animate-pulse mx-0.5">:</span>
                <span className="text-2xl sm:text-3xl font-bold text-midnight-300 font-mono tabular-nums tracking-wider self-end mb-0.5">
                  {seconds}
                </span>
              </div>
              ...
              {gregorianDate && hijriDate && (
                <div className="flex flex-col items-center gap-1 mt-2.5">
                  <div className="gold-divider w-28 mb-1.5" />
                  <span className="text-midnight-200 text-xs">
                    {gregorianDate.weekday.en}, {gregorianDate.day}{" "}
                    {gregorianDate.month.en} {gregorianDate.year}
                  </span>
                  <span className="text-gold-400 text-xs font-[Amiri] font-bold">
      ...
      {/* ===== CITY SELECTOR ===== */}
      <div className="relative z-40 max-w-xl mx-auto px-4 mb-4">
        <button
          onClick={() => setShowCitySelector(!showCitySelector)}
          className="w-full glass-card rounded-xl px-5 py-3 flex items-center justify-between hover:border-gold-500/20 transition-all duration-300 group cursor-pointer"
        >
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-gold-500/20 to-gold-600/10 flex items-center justify-center border border-gold-500/20">
            ...
            <div className="text-right">
              <span className="block text-base font-bold text-gold-400 font-[Amiri]">
                {selectedCity.nameAr}
              </span>
              <span className="block text-[11px] text-midnight-300">
      ...
      {nextPrayer && !loading && (
        <div className="max-w-xl mx-auto px-4 mb-4 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16">
            <div className="relative w-14 h-14">
              ...
              <CrescentMoon className="absolute inset-3 w-8 h-8 opacity-30" />
            </div>
            <p className="text-midnight-300 mt-4 font-[Amiri] text-base">
              جاري تحميل مواقيت الصلاة...
            </p>
          </div>
        ) : error ? (
          <div className="text-center py-10 glass-card rounded-2xl border border-red-500/20">
            <div className="text-4xl mb-3">⚠️</div>
            <p className="text-red-300 mb-2 text-sm">{error}</p>
            <button
              onClick={refetch}
              className="bg-gradient-to-r from-gold-500 to-gold-600 text-midnight-950 px-6 py-2 rounded-lg font-bold text-sm hover:from-gold-400 hover:to-gold-500 transition-all duration-300 cursor-pointer"
            >
              إعادة المحاولة
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {PRAYER_INFO.map((prayer, index) => {
              ...
              return (
                <div
                  key={prayer.key}
                  className={`relative rounded-2xl p-4 transition-all duration-500 border overflow-hidden
                    ${prayer.cssClass}
                ...
                >
                  ...
                  <div className="relative z-10 text-center">
                    <div className="text-2xl mb-1.5 drop-shadow-lg">{prayer.icon}</div>
                    <h3 className={`text-base font-bold font-[Amiri] mb-0.5 ${
                    ...
                    <div className={`w-7 h-[1px] mx-auto mb-2 ${
                    ...
                    {isNext && (
                      <span className="inline-block mt-2 text-[8px] bg-gold-500/15 text-gold-400 px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider border border-gold-500/20">
                        التالية
                      </span>
                    )}
                    {isCurrent && !isNext && (
                      <span className="inline-block mt-2 text-[8px] bg-teal-500/10 text-teal-400 px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider border border-teal-500/15">
                        الحالية
                      </span>
                    )}
                  </div>
      ...
      {/* ===== QURANIC VERSE ===== */}
      <div className="max-w-xl mx-auto px-4 mb-6 animate-fade-in-up" style={{ animationDelay: '600ms' }}>
        <div className="glass-card-light rounded-2xl p-5 text-center relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-gold-500/20 to-transparent" />
          <div className="text-gold-500/10 text-5xl font-[Amiri] leading-none mb-1">❝</div>
          <p className="text-gold-300 text-lg sm:text-xl font-[Amiri] leading-relaxed mb-2 px-4" dir="rtl">
            إِنَّ الصَّلَاةَ كَانَتْ عَلَى الْمُؤْمِنِينَ كِتَابًا مَوْقُوتًا
          </p>
          <div className="gold-divider w-16 mx-auto mb-2" />
          <p className="text-midnight-400 text-[11px] tracking-wider">
