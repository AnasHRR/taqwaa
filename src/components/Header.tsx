  ...
  return (
    ...
    >
      ...
      <span className="flex flex-col items-start leading-none">
        <span className="font-quran text-[22px] sm:text-[26px] font-bold leading-none text-gradient-gold">
          {t("common.appNameAr")}
        </span>
...
export function Header({ activePage, onNavigate, onOpenMore }: HeaderProps) {
  ...
  const navItems: Array<{ id: Page; label: string; icon: ReactNode }> = [
    { id: "home", label: t("nav.home"), icon: <Home size={18} strokeWidth={1.9} /> },
    { id: "quran", label: t("nav.quran"), icon: <BookOpen size={18} strokeWidth={1.9} /> },
    { id: "salaat", label: t("nav.prayer"), icon: <Compass size={18} strokeWidth={1.9} /> },
    { id: "dua", label: t("nav.azkar"), icon: <Heart size={18} strokeWidth={1.9} /> },
    { id: "about", label: t("nav.about"), icon: <Info size={18} strokeWidth={1.9} /> },
  ];
    ...
    >
      ...
      <div className="mx-auto flex h-[60px] sm:h-[68px] max-w-7xl items-center justify-between px-3.5 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 md:hidden">
          {activePage !== "home" ? (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-gold-500/25 bg-gold-50/90 px-3 py-1 text-xs font-bold text-gold-800 shadow-sm animate-fade-in">
              <span className="h-1.5 w-1.5 rounded-full bg-gold-500 animate-pulse" />
              {pageTitleMap[activePage]}
            </span>
          ) : (
            <span className="hidden xs:inline-flex items-center gap-1 rounded-full border border-gold-500/20 bg-white/80 px-2.5 py-1 text-[11px] font-semibold text-gold-700 shadow-sm">
              <Sparkles size={11} className="text-gold-600" />
              {t("common.moroccoKingdom")}
            </span>
          )}
        ...
        <div className="hidden items-center gap-5 md:flex lg:gap-7">
          ...
          <div className="border-s border-ink-200/80 ps-4">
            <LanguageSelector variant="desktop-header" />
          </div>
        </div>
      </div>
    </header>
